...
class BandarController extends Controller
{
    function index(Request $req){
        ...
        if($req->isMethod('post')) {
            ...
            $cari_kod_negeri = $req->cari_kod_negeri;
            $dae_kod_daerah = $req->dae_kod_daerah;
            $namamukim  = $req->namamukim;
            // dd($cari_kod_negeri);
            session([
                'cari_kod_negeri' => $cari_kod_negeri,
            ...
            if($req->has('page')) {
                $cari_kod_negeri = session('cari_kod_negeri');
                $dae_kod_daerah = session('dae_kod_daerah');
                $namamukim = session('namamukim');
                $queryType = 2;
            }
            else{
                session()->forget(['cari_kod_negeri', 'dae_kod_daerah', 'namamukim']);
            }            
        }
        ...
        if ($queryType == 1) {
            ...
            $query = \DB::table('ddsa_kod_bandar')
                        ...
                        ->select('ddsa_kod_bandar.*', 'ddsa_kod_daerah.dae_nama_daerah', 'ddsa_kod_negeri.neg_nama_negeri')
                        ->where(function($q) use ($cari_kod_negeri, $dae_kod_daerah, $namamukim){
                if(!empty($cari_kod_negeri)){
                    $q->where('ddsa_kod_negeri.neg_kod_negeri', $cari_kod_negeri);
                }    
